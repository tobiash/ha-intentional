...
### Fixed
- Home Assistant publication tests cover the shared authored-rule status projection contract.

## [0.12.9] - 2026-08-02

### Fixed
- Home Assistant entity publication reuses one authored-rule status projection when deriving rule and room entities instead of computing it twice.
...
### Added
- Tick runtime diagnostics split dispatch time across Reconciliation commit, Effects, finalization, and entity publication.

## [0.12.6] - 2026-08-02

### Added
- Intentional health diagnostics report last, average, and maximum Tick runtime stage durations for evaluation, alerting, persistence, Reconciliation, dispatch, lock waiting, and the complete cycle.

## [0.12.5] - 2026-08-02

### Fixed
- Release verification now uses the Engine's public state mapping when checking irrelevant state ingest.

## [0.12.4] - 2026-08-02

### Fixed
- Stable state updates from entities outside Intentional's ingest set no longer contend for the Tick runtime mutation lock.
...
### Fixed
- Singleton Alert Notifications use the rendered Alert summary as their title and description as their message instead of adding a hard-coded Intentional product prefix.
- Grouped Notifications use a dynamic Alert count as their title.
...
### Fixed
- **Automatic rollback health** clears inactive generation references after the stability window disarms rollback protection.
...
### Fixed
- **Home Assistant registry cleanup tests** register their config entry before creating linked room-control entities.

## [0.10.1] - 2026-07-12

### Fixed
- **Dynamic Home Assistant entities** wait for successful platform registration before publishing or removing state, preventing `NoEntitySpecifiedError` during rapid Rule and room-control changes.
- **Room-control registry cleanup** removes stale room status and pause entities while preserving current and user-disabled registry entries.
...
### Fixed
- **Live device-class membership** uses authoritative Home Assistant state-event metadata changes, avoiding stale semantic membership when registry refresh and state processing race.

## [0.9.2] - 2026-07-12

### Fixed
- **Home Assistant 2026.7 semantic fixtures** register config entries before device association and force attribute-only device-class state updates.

## [0.9.1] - 2026-07-12

### Fixed
- **Home Assistant 2026.7 semantic integration tests** use registry update APIs for area assignment and await asynchronous metadata membership convergence.
...
### Fixed
- **Document-validation coverage** now asserts normalized modifier syntax and distinguishes loader errors from later expression validation errors.

## [0.8.4] - 2026-07-12

### Fixed
- **Rule transaction coverage** now uses valid documents and verifies exact rollback after Store failures.
- **Registry invalidation** uses Home Assistant's synchronous callback contract while retaining burst coalescing.
- **Stable-Tick no-spam coverage** settles deferred setup persistence before measuring steady-state writes.

## [0.8.3] - 2026-07-12

### Fixed
- **Tick unload coordination** now exposes explicit unloading and idle state used to quiesce and resume the runtime safely.
- **Home Assistant integration tests** use supported service, state-machine, publication, and persistence seams on current HA releases.

## [0.8.2] - 2026-07-12

### Fixed
- **Single-Rule file patches** preserve the submitted YAML text while multi-Rule files still replace only the matching authored Rule.
- **Intentional-context state tests** now await the asynchronous serialized state listener.

## [0.8.1] - 2026-07-12

### Fixed
- **Bundled Reconciliation lookup** now exports the entry-scoped runtime key used by the Home Assistant integration and inspection APIs.
